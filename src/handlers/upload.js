...
export function registerUploadHandler(bot) {
  bot.on("document", async (message) => {
    try {
      await handleDocumentUpload(bot, message);
    } catch (error) {
  ...
  console.log("📦 Upload handler registered.");
}

async function handleDocumentUpload(bot, message) {
  ...
  const originalName = document.file_name || "website.zip";
  ...
  const maxBytes =
    Number(config.maxFileSizeMB || 50) * 1024 * 1024;

  const telegramFileSize = Number(document.file_size || 0);

  if (telegramFileSize > maxBytes) {
    await bot.sendMessage(
      chatId,
      `❌ File is too large.\n\n` +
        `Maximum allowed ZIP size: <b>${config.maxFileSizeMB} MB</b>\n` +
        `Your file: <b>${formatBytes(telegramFileSize)}</b>`,
      {
        parse_mode: "HTML"
      }
    );
  ...
  const tempDir = await fs.mkdtemp(
    path.join(os.tmpdir(), "telegram-hosting-")
  );
  ...
  try {
    ...
    const processed = await processProjectZip(
      tempZipPath,
      project.id
    );

    if (!processed?.sitePath) {
      throw new Error(
    ...
    await updateDeployment(
      project.id,
      {
        ...
        providerProject: cloudflareProjectName,
        url: deployment.url,
    ...
    await bot.sendMessage(
      ...
      [
        ...
        `🆔 <b>ID:</b> <code>${escapeHtml(project.id)}</code>`,
        "",
        "🌐 <b>Website URL:</b>",
        deployment.url,
        "",
...
/**
 * Edit Telegram status message safely.
 */
async function editStatus(
  ...
  try {
    ...
    // Telegram throws if the new text is identical.
    if (
...
/**
 * Make a safe temporary filename.
 */
function sanitizeFileName(fileName) {
  return String(fileName || "website.zip")
    .replace(/[^a-zA-Z0-9._-]/g, "_")
    .slice(0, 180);
}

/**
 * Format bytes for Telegram messages.
 */
...
/**
 * Convert internal errors into user-friendly messages.
 */
function getFriendlyErrorMessage(error) {
  ...
  if (
    lower.includes("cloudflare_api_token") ||
    lower.includes("cloudflare_account_id")
  ) {
  ...
  if (
    lower.includes("telegram file download")
  ) {
